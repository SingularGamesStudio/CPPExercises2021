...
void task2() {
    ...
    rassert(row1.size()==3, 5)
    rassert(array2d.size()==2, 6)
    ...
    array2d.push_back(vector<int>(10));
    for(int i = 0; i<10; i++){
        array2d.back()[i] = i;
    }
    print2DArray(array2d);
...
void task3() {
    ...
    cout << "input numbers of rows and cols(1-20): ";
    int x, y;
    cin >> x >> y;
    ...
    rassert(x<=20 && x>=1 && y>=1 && y<=20, "Error: data outside bounds");
    ...
    cout << "input a node (x, y) to be added 1, numbers <0 end the program\n";
    while (true) {
        int i1;
        int j1;
        cin >> i1 >> j1;
        if(i1<0 || j1<0)
            return;
        rassert(i1<x && j1<y, "numbers outside bounds");
        a[i1][j1]++;
        for(int i = 0; i<x; i++){
            for(int j = 0; j<y; j++){
                cout << a[i][j] << " ";
