...
cv::Mat sobelDXY(cv::Mat img) {
    ...
    int width = img.cols;
    Scalar color(0, 0);
    cv::Mat dxyImg(height, width, CV_32FC2, color); // в этой картинке мы сохраним обе производные:
...
cv::Mat convertDXYToGradientLength(cv::Mat img) {
    ...
    int width = img.cols;
    int height = img.rows;
    cv::Mat dImg(height, width, CV_32FC1); // создаем одноканальную картинку состоящую из 32-битных вещественных чисел
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            cv::Vec2f dxy = img.at<cv::Vec2f>(j, i);
            ...
            dImg.at<float>(j, i) = sqrt(y*y+x*x);
        }
    }
    return dImg;
}
